...
# constants
tf.app.flags.DEFINE_string("data_dir", "data/swda/full_swda_clean_12da_sentiment_dialog_corpus.p", "Raw data directory.")
...
tf.app.flags.DEFINE_string("test_path", "run1485114389", "the dir to load checkpoint for forward only")
...
def main():
    ...
    if not FLAGS.forward_only:
        config.description = raw_input("What is the purpose of this experiment?\n")

    valid_config = Config()
    valid_config.keep_prob = 1.0
    valid_config.dec_keep_prob = 1.0
    valid_config.batch_size = 60
    valid_config.use_grid = False
    ...
    if test_config.use_grid:
        assert test_config.latent_size == 2

    pp(config)

    # get data set
    api = JointSWDADialogCorpus(FLAGS.data_dir, word2vec=FLAGS.word2vec_path, word2vec_dim=config.embed_size)
    ...
    train_feed = JointSWDADataLoader("Train", train_dial, train_meta, config)
    valid_feed = JointSWDADataLoader("Valid", valid_dial, valid_meta, config)
    test_feed = JointSWDADataLoader("Test", test_dial, test_meta, config)
    ...
    with tf.Session() as sess:
        ...
        if not FLAGS.forward_only:
            ...
            for epoch in range(config.max_epoch):
                ...
                valid_feed.epoch_init(valid_config.batch_size, valid_config.backward_size,
                                      valid_config.step_size, shuffle=False, intra_shuffle=False)
                valid_model.valid("VALID", sess, valid_feed, marginal=True)
